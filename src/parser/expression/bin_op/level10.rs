...
use crate::parser::expression::bin_op::level9::higher_than_level9;
use crate::parser::expression::bin_op::{level9, BinOp};
...
pub(in crate::parser::expression) fn higher_than_level10(code: &str) -> IResult<&str, RValue> {
    alt((map(level9::parse, |binop| binop.into()), higher_than_level9))(code)
}
...
pub fn parse(code: &str) -> IResult<&str, BinOp> {
    map(
        tuple((
            higher_than_level10,
            many1(map(
                tuple((space0, tag("|"), space0, higher_than_level10)),
