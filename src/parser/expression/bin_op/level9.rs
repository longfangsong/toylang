...
use crate::parser::expression::bin_op::level8::higher_than_level8;
use crate::parser::expression::bin_op::{level8, BinOp};
...
pub(in crate::parser::expression) fn higher_than_level9(code: &str) -> IResult<&str, RValue> {
    alt((map(level8::parse, |binop| binop.into()), higher_than_level8))(code)
}
...
pub fn parse(code: &str) -> IResult<&str, BinOp> {
    map(
        tuple((
            higher_than_level9,
            many1(map(
                tuple((space0, tag("^"), space0, higher_than_level9)),
