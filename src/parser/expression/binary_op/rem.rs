...
impl RValue for Rem {
    fn generate_rvalue(&self) -> ExpressionParseResult {
        ...
        if left_result.type_name != right_result.type_name {
            panic!("Cannot rem two variables of different types!")
        }
        let typename = left_result.type_name;
        let bind_to = format!("rem_result_temp_{}", SEQUENCE.next());
        let code = format!("{} = srem {} {},{}", bind_to, typename, left_result.bind_to, right_result.bind_to);
...
pub fn parser_rem<'a>(input: (&'a str, Context<'a>)) -> IResult<(&'a str, Context<'a>), Rem> {
    map(lift(tuple((
        priority_0,
        space0,
        tag("%"),
        ...
        rvalue
            ...
            Rem { left: Box::new(left), right: Box::new(right) },
    )(input)
}